...
export default function AdminIdeasPage() {
  ...
  const loadItems = async () => {
    ...
    setError(null);
    try {
      const data = await adminApi.listFeedback({ type: 'idea', page, limit: PAGE_SIZE });
      setItems(data.items ?? []);
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Идеи</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Предложения и идеи от пользователей</p>
      </div>
              ...
              <Table className="divide-y divide-gray-200 text-sm dark:divide-gray-800">
                <TableHeader className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500 dark:bg-gray-900 dark:text-gray-400">
                  <TableRow>
                    ...
                    <TableCell isHeader className="px-3 py-2.5">Контакт</TableCell>
                  ...
                  {items.length === 0 ? (
                    ...
                    >
                      <TableCell className="px-3 py-3">
                        {!item.is_read && (
                          <span className="block h-2 w-2 rounded-full bg-brand-500" title="Новая" />
                        )}
                      </TableCell>
          ...
          <CardContent className="p-5">
            {detailLoading ? (
              ...
              <div className="space-y-4">
                ...
                {selected.telegram && (
                  <div>
                    <p className="text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">Контакт</p>
