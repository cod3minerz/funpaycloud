...
import { AlertTriangle, CreditCard, KeyRound, Lock, LogOut, Ticket } from '@/shared/streamline/icons';
...
export default function PlatformFrame({ children, basePath, scopeClassName }: PlatformFrameProps) {
  ...
  const lockAllowedPath = pathname === `${basePath}/subscription` || pathname === `${basePath}/promo-codes`;
  ...
  async function handleCreateFirstAccount(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (Number.isInteger(createdID) && createdID > 0) {
        router.push(`${basePath}/accounts?connectProxy=${createdID}`);
      } else {
      ...
      <div className="platform-scope flex min-h-[100dvh] items-center justify-center p-4" data-theme={theme}>
        <section className="w-full max-w-[680px] rounded-2xl border border-red-500/35 bg-[var(--pf-surface)] p-6 shadow-[0_0_0_1px_rgba(239,68,68,0.15),0_28px_80px_rgba(239,68,68,0.12)]">
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            <Link href={`${basePath}/subscription`} className="platform-btn-primary inline-flex justify-center gap-1.5">
              <CreditCard size={14} />
              Продлить подписку
            </Link>
            <Link href={`${basePath}/promo-codes`} className="platform-btn-secondary inline-flex justify-center gap-1.5">
              <Ticket size={14} />
  ...
  return (
    <div className="relative">
      <div
        className={`platform-scope ${scopeClassName ?? ''} platform-shell ${sidebarCollapsed ? 'platform-shell--collapsed' : 'platform-shell--expanded'} ${zeroAccountLocked ? 'pointer-events-none select-none blur-[2px] sm:blur-[3px]' : ''}`}
        data-theme={theme}
      >
        <Sidebar collapsed={sidebarCollapsed} theme={theme} basePath={basePath} />
        ...
        <Sidebar mobile open={mobileSidebarOpen} onClose={() => setMobileSidebarOpen(false)} theme={theme} basePath={basePath} />
          ...
          {showRenewBanner ? (
            <div className="border-b border-red-500/35 bg-red-500/12 px-4 py-2.5 text-[13px] text-red-700 sm:px-6">
              <div className="mx-auto flex max-w-[1400px] flex-wrap items-center justify-between gap-2">
                ...
                <Link href={`${basePath}/subscription`} className="platform-btn-primary h-8 px-3 text-xs">
                  Продлить
                </Link>
              </div>
            </div>
